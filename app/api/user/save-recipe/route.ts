...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log('[save-recipe] Decoded userId:', userId);
    } catch (err) {
      console.error('[save-recipe] Token verification failed:', err);
      return NextResponse.json(
    ...
    const isSaved = savedRecipes.some((id: ObjectId) => id.toString() === recipeId);

    if (isSaved) {
      // Unsave - remove from savedRecipes
      await db.collection('users').updateOne(
        { _id: new ObjectId(userId) },
        { $pull: { savedRecipes: new ObjectId(recipeId) } } as any
      );
    ...
    // Get updated user savedRecipes
    const updatedUser = await db.collection('users').findOne({
      _id: new ObjectId(userId)
    });

    console.log('[save-recipe] Success:', { isSaved: !isSaved, savesCount: updatedRecipe?.savesCount || 0 });

    return NextResponse.json({
      success: true,
      isSaved: !isSaved,
      savesCount: updatedRecipe?.savesCount || 0,
      savedRecipes: (updatedUser?.savedRecipes || []).map((id: ObjectId) => id.toString())
    });
  } catch (error: any) {
